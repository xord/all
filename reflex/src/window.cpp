...
namespace Reflex
{
	...
	static void
	update_flag (Window::Data* self, uint flag, bool add)
	{
		if (add)
			Xot::add_flag(&self->flags, flag);
		else
			Xot::remove_flag(&self->flags, flag);
	}
	...
	update_pointer_through (Window* window)
	{
		...
		{
			update_flag(self, Data::POINTER_THROUGH_PAUSED, opaque);
		...
		update_flag(self, Data::POINTER_THROUGH_ENTERED, entered);
