...
module Reflex
  ...
  class WebView
    ...
    def clear_find()
      @find_state = nil
      eval_js! 'window.__REFLEX__ && __REFLEX__.search && __REFLEX__.search.clear()'
      self
    end

    # Runs a __REFLEX__.search call and delivers its {count, index} to the
    # block as a FindResult.
    private def find__(call, &block)
      js = "window.__REFLEX__ && __REFLEX__.search ? __REFLEX__.search.#{call} : {count: 0, index: 0}"
      eval_js(js) do |r|
        r ||= {}
